...
const char* getDeviceInfoString(u32 location) {
	...
	if(!strcmp(topStr, "Unknown") && getExiIdByLocation(location, &exi_id)) {
		sprintf(topStr, "Desconocido (0x%08X)", exi_id);
	}
	return topStr;
}
	...
	if(!page_num) {
		...
		if(!strncmp(IPLInfo, "(C) ", 4)) {
			if((SYS_GetConsoleType() & SYS_CONSOLE_MASK) == SYS_CONSOLE_RETAIL) {
				...
				sprintf(topStr, "%.*s (Modo %s)", 0x11, IPLInfo[0x55] ? &IPLInfo[0x55] : "DEV  Revision 1.0", swissSettings.sramBoot ? "Produccion" : "Desarrollo");
			}
		}
		else {
			strcpy(topStr, "Ficticia");
		}
		...
		if((pvr & 0xFFFFF000) == 0x00083000) {
			...
			sprintf(topStr, "Desconocido (0x%08X)", pvr);
		}
		...
		if((chipId & 0xFFFFFFF) == 0x46500B1) {
			...
			sprintf(topStr, "Desconocido (0x%08X)", chipId);
		}
		...
		DrawAddChild(container, DrawStyledLabel(640/2, 250, (char*)"INTERFAZ DVD", 0.65f, true, defaultColor));
		...
		DrawAddChild(container, DrawStyledLabel(640/2, 306, (char*)(getDTVStatus() ? getRawDTVStatus() ? "Activado" : "Forzado" : "Desactivado"), 0.75f, true, defaultColor));
		...
		DrawAddChild(container, DrawStyledLabel(640/2, 140, "por emu_kidid y Extrems, 2025", 0.75f, true, defaultColor));
		sprintf(topStr, "Cambios %s; Revision %s", GIT_COMMIT, GIT_REVISION);
		DrawAddChild(container, DrawStyledLabel(640/2, 165, topStr, 0.75f, true, defaultColor));
		DrawAddChild(container, DrawStyledLabel(640/2, 190, "Compilado con " _V_STRING, 0.64f, true, defaultColor));
		DrawAddChild(container, DrawStyledLabel(640/2, 240, "Codigo/Actualizaciones/Asuntos", 0.75f, true, defaultColor));
		DrawAddChild(container, DrawStyledLabel(640/2, 264, "github.com/emukidid/swiss-gc", 0.64f, true, defaultColor));
		DrawAddChild(container, DrawStyledLabel(640/2, 288, "github.com/lopeztutoriales/swiss-gc-esp", 0.64f, true, defaultColor));
