...
int find_existing_entry(char *entry, bool load) {
	...
	if(entryDevice) {
		print_debug("Dispostivo requerido para entrada [%s]\n", entryDevice->deviceName);
...
bool deleteFileOrDir(file_handle* entry) {
	if(entry->fileType == IS_DIR) {
		print_debug("Entrando en dir. para borrar: %s\n", entry);
		...
		print_debug("Finalmente, borrando directorio vacio: %s\n", entry);
		return !devices[DEVICE_CUR]->deleteFile(entry);
	}
	if(entry->fileType == IS_FILE) {
		print_debug("Borrando archivo: %s\n", entry);
		return !devices[DEVICE_CUR]->deleteFile(entry);
	}
