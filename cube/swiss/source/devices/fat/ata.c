...
int _ideExiVersion(int chn) {
	...
	print_debug("ID IDE-EXI: %08X\n",cid);
	if((cid&~0xff)==EXI_IDE_ID) {
		print_debug("IDE-EXI v2+ detectado\n");
		return (cid&0xff)-'1';
	}
	else {
		print_debug("Desconocido - se supone que es IDE-EXI v1\n");
		return IDE_EXI_V1;
	}
}
...
u32 _ataDriveIdentify(int chn) {
	...
	do {
		...
		retries--;
		print_debug("(%08X) Esperando que BSY se despeje..\n", tmp);
	}
	while((tmp & ATA_SR_BSY) && retries);
	if(!retries) {
		print_debug("Reintentos superados..\n");
		return -1;
	}
	...
	do { 
		...
		usleep(100000);	//sleep for 0.1 seconds
		retries--;
		print_debug("(%08X) Esperando por DRQ para cambiar..\n", tmp);
	}
	while((!(tmp & ATA_SR_DRQ)) && retries);
	if(!retries) {
		print_debug("(%08X) La unidad no responde, fallo el inicio de IDE-EXT..\n", tmp);
		return -1;
	}
	usleep(2000);
	...
	print_debug("HDD %d GB Conectado\n", ataDriveInfo.sizeInGigaBytes);
	print_debug("Modo LBA 48-Bit %s\n", ataDriveInfo.lba48Support ? "Soportado" : "No Soportado");
	if(!ataDriveInfo.lba48Support) {
		print_debug("Cilindros: %i\n",ataDriveInfo.cylinders);
		print_debug("Cabezas por Cilindro: %i\n",ataDriveInfo.heads);
		print_debug("Sectores Por Pista: %i\n",ataDriveInfo.sectors);
	}
	print_debug("Modelo: %s\n",ataDriveInfo.model);
	print_debug("N. Serie: %s\n",ataDriveInfo.serial); 
...
int ataUnlock(int chn, int useMaster, char *password, int command)
{
	...
	do {
		...
		usleep(100000);	//sleep for 0.1 seconds
		retries--;
		print_debug("DESBLOQUEAR (%08X) Esperando que BSY se despeje..\n", tmp);
	}
	while((tmp & ATA_SR_BSY) && retries);
	if(!retries) {
		print_debug("DESBLOQUEAR Reintentos excedidos..\n");
		return -1;
	}
	...
	do { 
		...
		usleep(100000);	//sleep for 0.1 seconds
		retries--;
		print_debug("DESBLOQUEAR (%08X) Esperando por DRQ para cambiar..\n", tmp);
	}
	while((!(tmp & ATA_SR_DRQ)) && retries);
	if(!retries) {
		print_debug("DESBLOQUEAR (%08X) La unidad no responde, fallo el inicio de IDE-EXT..\n", tmp);
		return -1;
	}
	usleep(2000);
...
int ataWriteSectors(int chn, u64 sector,unsigned int numSectors, unsigned char *src) 
{
	...
	while(numSectors) {
		if((ret=_ataWriteSector(chn,sector,(u32*)src))) {
			print_debug("(%08X) Fallo al escribir!..\n", ret);
			return -1;
		}
		src+=512;
		sector++;
