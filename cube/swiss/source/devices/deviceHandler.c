...
void print_frag_list(file_frag *fragList, u32 totFrags) {
	print_debug("== Lista de Fragmentos ==\n");
	for(int i = 0; i < totFrags; i++) {
		print_debug("Frag %i: ofs en arch: [0x%08X] long. [0x%08X] LBA en disco [0x%012llX]\n",
			i, fragList[i].offset, fragList[i].size, fragList[i].fileBase);
	}
	print_debug("== Fin de Fragmentos ==\n");
}
