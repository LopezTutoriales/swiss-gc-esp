...
void *installPatch(int patchId) {
	...
	print_debug("Parche %i instalado en %08X\n", patchId, patchLocation);
...
void *getPatchAddr(int patchId) {
	if(patchId >= PATCHES_MAX || patchId < 0) {
		print_debug("Localizacion de parche solicitada invalida\n");
		return NULL;
	}
...
int install_code(int final)
{
	...
	// SD Card over EXI
		...
		print_debug("Instalando Parche para SD sobre EXI\n");
	}
	// DVD
		...
		print_debug("Instalando Parche para DVD\n");
	}
	// USB Gecko
		...
		print_debug("Instalando Parche para USB Gecko\n");
	}
	// Wiikey Fusion
		...
		print_debug("Instalando Parche para WKF\n");
	}
	// Broadband Adapter
		...
		print_debug("Instalando Parche para FSP\n");
	}
	// GC Loader
		...
		print_debug("Instalando Parche para GC Loader\n");
	}
	// FlippyDrive
		...
		print_debug("Instalando Parche para FlippyDrive\n");
	}
	if (!final) {
		print_debug("Espacio para parche restante: %i\n", top_addr - LO_RESERVE);
		print_debug("Espacio tomado por parches de vars/video: %i\n", HI_RESERVE - top_addr);
...
bool find_pattern(u32 *data, int dataType, u32 offsetFoundAt, u32 length, FuncPattern *functionPattern)
{
	...
	if (!functionPattern) {
		print_debug("Longitud: %d\n", FP.Length);
		print_debug("Cargas: %d\n", FP.Loads);
		print_debug("Almacenamiento: %d\n", FP.Stores);
		print_debug("LlamadasF: %d\n", FP.FCalls);
		print_debug("Rama: %d\n", FP.Branch);
		print_debug("Movimientos: %d\n", FP.Moves);
	}
...
int Patch_FontEncode(u32 *data, u32 length)
{
	...
	print_debug("Parche:[fontEncode] aplicado %d veces\n", patched);
	return patched;
}
	...
	if (!is_gamecube() && (!strncmp(gameID, "GZLP01", 6) || !strncmp(gameID, "GZLE01", 6) || !strncmp(gameID, "GZLJ01", 6)) && dataType == PATCH_DOL) {
		...
		while(addr_start<addr_end) 
		{
			if(mode == 1 && memcmp(addr_start,GXSETVAT_PAL_orig,sizeof(GXSETVAT_PAL_orig))==0) 
			{
				...
				print_debug("Parcheado:[__GXSetVAT] PAL\n");
				patched=1;
			}
			if(mode == 2 && memcmp(addr_start,GXSETVAT_NTSC_orig,sizeof(GXSETVAT_NTSC_orig))==0) 
			{
				...
				print_debug("Parcheado:[__GXSetVAT] NTSC\n");
				patched=1;
			}
			addr_start += 4;
		}
...
void *Calc_ProperAddress(void *data, int dataType, u32 offsetFoundAt) {
	if(dataType == PATCH_DOL || dataType == PATCH_DOL_PRS) {
		...
		if (hdr->textOffset[0] != DOLHDRLENGTH) {
			print_debug("Header de DOL parece invalido %08X\n",hdr->textOffset[0]);
			return NULL;
		}
...
int Patch_CheatsHook(u8 *data, u32 length, u32 type) {
	...
	for( i=0; i < length; i+=4 )
	{
			...
			(*(vu32*)(data+i+8) == 0x38000004 || *(vu32*)(data+i+8) == 0x808400E4)) 
		{
			...
			if(properAddress) {
				print_debug("Encontrado:[Hook:OSSleepThread] @ %08X\n", properAddress );
	...
	for( i=0; i < length; i+=4 )
	{
			...
			*(vu32*)(data+i+40) == 0x90058000  ) )
		{
			...
			void *properAddress = Calc_ProperAddress(data, type, i+j);
			if(properAddress) {
				print_debug("Encontrado:[Hook:GXDrawDone] @ %08X\n", properAddress );
				*(vu32*)(data+i+j) = branch(CHEATS_ENGINE_START, properAddress);
				break;
			}
		}
