...
void wait_for_ack () {
        unsigned char ack;
        if (gecko_read (&ack, 1))
                fprintf (stderr, "\nerror recibiendo el ack\n");
        else if (ack != 0xaa)
                fprintf (stderr, "\nack desconocido (0x%02x)\n", ack);
}
...
void cache_path() {
	...
	if (dir != NULL) {
		...
		while ((ent = readdir (dir)) != NULL) {
			...
			stat(path, &fstat);
			printf ("%s %i [%s]\n", ent->d_name, fstat.st_size, S_ISDIR(fstat.st_mode) ? "DIR":"ARCHIVO");
		...
		fprintf (stderr, "Imposible leer directorio [%s]\n",getCurPath());
		exit(EXIT_FAILURE);
	}
}

void send_file_data(usb_data_req *req) {
	printf("Leer archivo: offset %08X, tam %i          \r", req->offset, req->size);
	if(served_file_fp) {
...
int main (int argc, char **argv) {
        ...
        printf ("swissserver " SWISSSERVER_VERSION "\n"
                "hecho por emu_kidid para Swiss + USB Gecko\n\n");
        ...
        if (!tty) {
                fprintf (stderr, "Configure variable de entorno %s a "
                         "tu usbgecko "
                         "tty dispositivo (ej \"/dev/ttyUSB0\")"
                         "\n", envvar);
                exit (EXIT_FAILURE);
        }
...
#endif
        if (gecko_open (tty)) {
                fprintf (stderr, "Imposible abrir dispositivo\n");
                exit (EXIT_FAILURE);
        }
		...
		while(1) {
			...
			if(resp == ASK_READY) {
				...
				printf("Obtuve archivo bloqueado (%02X) solicitud desde GC\n",resp);
				gecko_send_byte(&ANS_READY);
				while(1) {
					// Read file, offset and size requested by GC
					usb_data_req req;
				...
				gecko_read_byte();
				printf("Desbloquear llamado cuando ya esta desbloqueado, eso esta bien.\n");
			}
			else {
				printf("Respuesta desconocida desde GC!\n");
				exit(1);
			}
		}
